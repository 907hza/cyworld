...
public class Paging implements Serializable
{
	...
	public Paging(long totalCount, long listCount, long pageCount, long curPage) // 값 세팅을 위한 생성자 (매개변수 필수)
	{ // 페이징 처리를 위해 필요한 필수 값 : 전체 페이지 수, 한 페이지에서의 게시물 수, 페이징 범위의 수, 현재 페이지
		...
		if(totalCount > 0)
			pagingProc(); // 전체 페이지 갯수가 1개이상 있으면 페이징 계산 프로세스를 이용해서 값을 설정해준다
	}
